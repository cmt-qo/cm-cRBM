...
int main(){
    ...
    if (exc){
    	std::cout<<"Computing the cRBM expectation values <Psi|W(m=4)|Psi> of the first excited state |Psi> for the field h in direction ("<<hx<<", "<<hy<<", "<<hz<<")"<<std::endl;
        std::cout<<"in the range 0.1<h<0.61 in steps of size delta_h=0.01..."<<std::endl;
        std::cout<<std::endl;
    }
    else{
        std::cout<<"Computing the cRBM expectation values <Psi|W(m=4)|Psi> of the ground state |Psi> for the field h in direction ("<<hx<<", "<<hy<<", "<<hz<<")"<<std::endl;
    ...
    fileNameStream_b<<"expvalues_AsBp.txt";
    ...
    for (int h_i=start_h_i; h_i<n_hvalues; h_i++){
      ...
      //create AsBp operator
      AsBp Wloop(L,1);


      Sampler<cRBM,AsBp> sampler1(Wf,Wloop,-1);
      ...
      double expvalue, var;
      sampler1.OutputEnergy(expvalue,var);
       

      file_b<<h<<" "<<-expvalue<<std::endl;
      
      std::cout<<"Field strength: h="<<h<<". Computed cRBM expectation value: <W(m=4)>="<<-expvalue<<" +/- "<<var<<std::endl;  //put here MC error
      std::cout<<std::endl;
